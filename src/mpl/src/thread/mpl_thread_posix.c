...
void MPL_thread_set_affinity(MPL_thread_id_t thread, int *affinity_arr, int affinity_size,
                             int *errp)
{
#ifdef MPL_HAVE_PTHREAD_SETAFFINITY_NP
    ...
    if ((pthread_err = pthread_setaffinity_np(thread, sizeof(cpu_set_t), &cpuset)) != 0) {
        MPL_error_printf("pthread_setaffinity_np failed\n");
        err = MPL_ERR_THREAD;
        goto fn_exit;
    }

    if ((pthread_err = pthread_getaffinity_np(thread, sizeof(cpu_set_t), &cpuset)) != 0) {
        MPL_error_printf("pthread_getaffinity_np failed\n");
        err = MPL_ERR_THREAD;
        goto fn_exit;
    }
    ...
    if (set_size != affinity_size) {
        MPL_error_printf("affinity is not set\n");
        err = MPL_ERR_THREAD;
        goto fn_exit;
    }
